...
import Text.Pandoc.Definition (Pandoc(..), Block(..), Inline(..), nullMeta)

main :: IO ()
main = defaultMain tests

tests :: TestTree
tests = testGroup "JSONSchema Tests"
  [ testGroup "Schema Structure Tests"
      [ testSchemaStructure
      , testOperationNames
      , testFocusSchema
      ]
  ...
  , testGroup "Obvious Failure Cases"
      [ testInvalidOperationNames
      , testMissingRequiredFields
      , testWrongTypes
      ]
  , testGroup "Subtle Failure Cases"
      [ testExtraFields
      , testFocusConfusion
...
testValidOperations = testGroup "Valid operations parse correctly"
...
testValidFocusTypes = testGroup "Valid focus types"
...
testInvalidOperationNames = testGroup "Invalid operation names fail"
...
testMissingRequiredFields = testGroup "Missing required fields fail"
...
testWrongTypes = testGroup "Wrong field types fail"
...
testExtraFields = testGroup "Extra fields handling"
...
testFocusConfusion = testGroup "Focus type confusion"
...
testArrayCountMismatch = testGroup "Array count validation"
...
testInvalidEnumValues = testGroup "Invalid enum values"
...
testSchemaWithRealOperations = testGroup "Schema works with real operations"
  [ testCase "Parse and apply valid JSON operations" $ do
      let doc = Pandoc nullMeta [Para [Str "Hello"], Para [Str "World"]]
          jsonOps = encode [
            ...
            object [ "op" .= ("replace" :: Text), "focus" .= object ["index" .= (0 :: Int)],
...
testOperationArraySchema = testGroup "Operation array schema"
